...
def testsida():
    vem= {'username': 'YO'}
    return render_template('testsida.html', title= 'Testtest', vem=vem)
...
def user(username):
    ...
    page=request.args.get('page', 1, type=int)
    posts = user.post.order_by(Post.timestamp.desc()).paginate(page, app.config['POSTS_PER_PAGE'], False)
...
def follow(username):
    ...
    if form.validate_on_submit():
        ...
        if user==current_user:
            flash('impossible to follow yourself')
...
def unfollow(username):
    ...
    if form.validate_on_submit():
        ...
        if user==current_user:
            flash('impossible to follow or unfollow yourself')
...
def explore():
    page=request.args.get('page', 1, type=int)
    posts=Post.query.order_by(Post.timestamp.desc()).paginate(page, app.config['POSTS_PER_PAGE'], False) #global variable see app.config
